...
export async function getModuleDetails(moduleId: string): Promise<MarketplaceModule> {
  const res = await fetch(`${MARKETPLACE_API}/modules/${moduleId}`);
  if (!res.ok) throw new Error("Module not found");
  return res.json();
}

export async function installModule(moduleId: string, siteId: string): Promise<ModuleManifest> {
...
export async function uninstallModule(moduleId: string, siteId: string): Promise<void> {
...
export async function checkForUpdates(): Promise<Array<{ moduleId: string; currentVersion: string; latestVersion: string }>> {
