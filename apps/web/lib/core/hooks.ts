...
class HookSystem {
  ...
  private addToMap(map: Map<string, HookCallback[]>, hook: string, callback: HookCallback, _priority: number): void {
    if (!map.has(hook)) map.set(hook, []);
    map.get(hook)!.push(callback);
  }

  private removeFromMap(map: Map<string, HookCallback[]>, hook: string, callback: HookCallback): void {
...
export function addFilter(hook: string, callback: HookCallback, priority?: number): void {
