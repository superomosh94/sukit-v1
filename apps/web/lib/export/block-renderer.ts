...
const blockRenderers: Record<string, (b: { props: Record<string, unknown>; styles?: Record<string, unknown>; id?: string }) => string> = {
  textBlock: (b) => {
    const tag = (b.props.tag as string) || 'p';
    const content = (b.props.content as string) || '';
    const style = inlineStyle(b.styles, { margin: '0 0 0.75rem', lineHeight: '1.6' });
  ...
  headingBlock: (b) => {
    ...
    const style = inlineStyle(b.styles, { margin: '0 0 0.5rem', fontWeight: '700', fontSize: sizes[level] ?? '2rem', lineHeight: '1.2' });
  ...
  buttonBlock: (b) => {
    ...
    const sizes: Record<string, string> = { sm: '0.5rem 1rem', md: '0.75rem 1.5rem', lg: '1rem 2rem' };
  ...
  formBlock: (b) => {
    ...
    const fields = (b.props.fields as Array<{ type: string; label: string; name: string; required?: boolean; placeholder?: string }>) || [];
    ...
    const fieldsHtml = fields.map((f) => {
      ...
      const label = f.label ? `<label style="display:block;margin-bottom:0.25rem;font-weight:500">${f.label}</label>` : '';
      if (f.type === 'textarea') {
  ...
  containerBlock: (b) => {
    ...
    const style = inlineStyle(b.styles, { margin: '0 auto', maxWidth, width: '100%' });
  ...
  gridBlock: (b) => {
    ...
    const children = (b.props.children as string) || '';
    const style = inlineStyle(b.styles, { display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap });
    return `<div style="${style}">${children}</div>`;
  },

  accordionBlock: (b) => {
    const items = (b.props.items as Array<{ title: string; content: string }>) || [];
    const style = inlineStyle(b.styles, { borderTop: '1px solid #e2e8f0' });
    return `<div style="${style}">${items.map((item, i) => `
  ...
  tabsBlock: (b) => {
    const tabs = (b.props.tabs as Array<{ label: string; content: string }>) || [];
    const style = inlineStyle(b.styles, {});
  ...
  carouselBlock: (b) => {
    ...
    const style = inlineStyle(b.styles, { position: 'relative', overflow: 'hidden' });
  ...
  testimonialBlock: (b) => {
    ...
    const style = inlineStyle(b.styles, { padding: '1.5rem', borderRadius: '8px', backgroundColor: '#f8fafc' });
  ...
  pricingBlock: (b) => {
    ...
    const style = inlineStyle(b.styles, { padding: '1.5rem', borderRadius: '8px', border: featured ? '2px solid #0f172a' : '1px solid #e2e8f0', backgroundColor: featured ? '#f8fafc' : '#fff', textAlign: 'center' as const });
    return `<div style="${style}">
  ...
  faqBlock: (b) => {
    const items = (b.props.items as Array<{ question: string; answer: string }>) || [];
    const style = inlineStyle(b.styles, { borderTop: '1px solid #e2e8f0' });
  ...
  listBlock: (b) => {
    ...
    const style = inlineStyle(b.styles, { paddingLeft: '1.5rem', margin: '0.5rem 0' });
...
function renderUnknownBlock(b: { props: Record<string, unknown>; blockType: string; id?: string }): string {
...
function inlineStyle(styles?: Record<string, unknown>, defaults?: Record<string, string>): string {
