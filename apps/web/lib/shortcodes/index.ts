type ShortcodeHandler = (attrs: Record<string, string>, content?: string) => string;
...
export function parseShortcodes(input: string): string {
  return input.replace(/\[(\w+)([^\]]*)\](.*?)\[\/\1\]|\[(\w+)([^\]]*)\]/gs, (match, tag1, attrs1, content, tag2, attrs2) => {
    const tag = tag1 || tag2;
    ...
    const attrRegex = /(\w+)\s*=\s*"([^"]*)"|(\w+)\s*=\s*'([^']*)'|(\w+)\s*=\s*(\S+)|(\w+)/g;
    let m;
...
registerShortcode("date", (attrs) => {
  ...
  const d = new Date();
  return format.replace("YYYY", d.getFullYear().toString()).replace("MM", String(d.getMonth() + 1).padStart(2, "0")).replace("DD", String(d.getDate()).padStart(2, "0"));
});

registerShortcode("site_url", () => "/");

registerShortcode("gallery", (attrs, content) => {
  const ids = (attrs.ids || content || "").split(",").map(s => s.trim()).filter(Boolean);
  const cols = parseInt(attrs.columns || "3");
  const size = attrs.size || "150px";
  if (ids.length === 0) return "";
  const items = ids.map(id => `<img src="/api/media/${id}" alt="" style="width:${size};height:${size};object-fit:cover;border-radius:4px" />`).join("");
...
registerShortcode("button", (attrs, content) => {
  ...
  const variants: Record<string, string> = { primary: "#0f172a;color:#fff", secondary: "#e2e8f0;color:#0f172a", outline: "transparent;color:#0f172a;border:1px solid #0f172a" };
