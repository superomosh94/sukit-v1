...
export async function GET(req: Request, { params }: { params: Promise<{ postId: string }> }) {
  ...
  const post = await prisma.post.findUnique({
    where: { id: postId },
    include: { author: { select: { name: true, image: true, email: true } }, taxonomies: { include: { taxonomy: true } }, revisions: { orderBy: { version: "desc" }, take: 10 } },
  });
...
export async function PUT(req: Request, { params }: { params: Promise<{ postId: string }> }) {
  ...
  if (taxonomyIds) {
    ...
    await prisma.postTaxonomy.createMany({ data: taxonomyIds.map((tid: string) => ({ postId, taxonomyId: tid })) });
  }

  await prisma.postRevision.create({
    data: { postId, version: (existing as any)._revisionCount || 1, title: existing.title, content: existing.content || "", excerpt: existing.excerpt || "" },
  });
