...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ redirects: (option?.value as RedirectRule[]) || [] });
}
...
export async function POST(req: Request) {
  ...
  if (!siteId || !source || !target) {
    return NextResponse.json({ error: "siteId, source, target required" }, { status: 400 });
  }
  ...
  const rules: RedirectRule[] = (existing?.value as RedirectRule[]) || [];
  ...
  const option = await prisma.option.upsert({
    where: { key_siteId: { key, siteId } },
    update: { value: rules },
    create: { key, value: rules, siteId },
  });

  return NextResponse.json({ redirects: option.value as RedirectRule[] }, { status: 201 });
}
...
export async function DELETE(req: Request) {
  ...
  if (!siteId || !source) {
    return NextResponse.json({ error: "siteId, source required" }, { status: 400 });
  }
  ...
  const rules: RedirectRule[] = (existing.value as RedirectRule[]).filter(
    (r) => r.source !== source,
  );
