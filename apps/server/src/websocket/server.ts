...
export async function createSocketServer(
  ...
  if (process.env.REDIS_URL) {
    try {
      ...
      pubClient.on('end', noop);
      const subClient = pubClient.duplicate();
      subClient.on('error', noop);
      subClient.on('end', noop);
      ...
      logger.warn(
        'Redis not available — running without multi-instance support'
      );
    }
  }
  ...
  collab.on('connection', (socket) => {
    ...
    logger.info(`User connected to collaboration namespace`, {
      userId,
      socketId: socket.id,
    });
