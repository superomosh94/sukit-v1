...
export function createUIAPI() {
  const slots = new Map<string, Array<{ component: ComponentType<any>; options: SlotOptions; id: string }>>();

  return {
    registerSlot(slot: string, component: ComponentType<any>, options: SlotOptions = {}): void {
    ...
    renderSlot(slot: string, props?: any): ReactNode {
      ...
      return components.map(({ component: Component, id }) => <Component key={id} {...props} />);
    },
